...
func TestParseFieldPathWithGroups(t *testing.T) {
	testCases := []struct {
		input    string
		expected []interface{}
	}{
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			result := parseFieldPathWithGroups(tc.input, nil)
			assert.Equal(t, tc.expected, result)
		})
	}
}
...
func TestParseFieldPathEdgeCases(t *testing.T) {
	t.Run("negative index should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[-1].name", nil)
	...
	t.Run("very large index should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[18446744073709551615].name", nil) // math.MaxUint64
		expected := []interface{}{}                                                  // Should return empty slice for invalid path
		assert.Equal(t, expected, result)
	})

	t.Run("index exceeding maxSliceIndex should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[1000000].name", nil) // equals maxSliceIndex
		expected := []interface{}{}                                     // Should return empty slice for invalid path
		assert.Equal(t, expected, result)
	})

	t.Run("valid large index under limit should work", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[999999].name", nil) // under maxSliceIndex
	...
	t.Run("non-numeric index should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[abc].name", nil)
		expected := []interface{}{} // Should return empty slice for invalid path
		assert.Equal(t, expected, result)
	})

	t.Run("empty brackets should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[].name", nil)
		expected := []interface{}{} // Should return empty slice for invalid path
		assert.Equal(t, expected, result)
	})

	t.Run("missing closing bracket should return empty slice", func(t *testing.T) {
		result := parseFieldPathWithGroups("items[123", nil)
...
func TestCompleteCoverage(t *testing.T) {
	t.Run("bindNestedFormFieldWithGroups legacy function", func(t *testing.T) {
		target := struct {
			Items []string `form:"items"`
		}{}
		val := reflect.ValueOf(&target).Elem()
		...
		// Test the function directly for coverage
		err := bindNestedFormFieldWithGroups(val, typ, "items[0]", []string{"test"}, nil)
		assert.NoError(t, err)
		assert.Len(t, target.Items, 1)
		assert.Equal(t, "test", target.Items[0])
	})
	...
	t.Run("parseFieldPathWithGroups legacy behavior", func(t *testing.T) {
		// Test the parseFieldPathWithGroups function directly
		result := parseFieldPathWithGroups("items[0].name", nil)
		expected := []interface{}{"items", 0, "name"}
		assert.Equal(t, expected, result)

		// Test with invalid index in function
		result = parseFieldPathWithGroups("items[-1].name", nil)
...
func TestCompleteHundredPercentCoverage(t *testing.T) {
	t.Run("bindNestedFormFieldWithGroups with empty parts", func(t *testing.T) {
		target := struct {
			Items []string `form:"items"`
		}{}
		val := reflect.ValueOf(&target).Elem()
		...
		err := bindNestedFormFieldWithGroups(val, typ, "items[-1]", []string{"test"}, nil)
